...
for i in range(8):
    ...
    # Set the output filename to the current timestamp
        ...
        subprocess.run(["ffmpeg", "-i", output_filename,"-c:v", "libx264", "-preset", "slow", "-crf", "22", "-an", mp4_output_filename])
        ...
        os.rename(mp4_output_filename, done_directory + mp4_filename)
        print(f"Recorded: {done_directory}{mp4_filename}", file=sys.stdout, flush=True)
